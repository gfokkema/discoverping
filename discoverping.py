...
class DiscoverPing:
    ...
    def done(self):
        # FIXME: It can happen that input and output are empty, while items are being processed.
        return self.input_done() and self.output_done()
    ...
    def run(self, hostnames=[], repeat=1, num_threads=10):
        self._threads = icmptrace.threads(num_threads, self._input, self._output)
    ...
    def process(self):
        ...
        for cur, attrib in reversed([x for src in srcs for x in src.items()]):
            ...
            if cur and attrib:
                ...
                if prev and prev not in ['192.168.5.1', '192.168.69.1', '192.168.178.1']:
...
if __name__ == '__main__':
    ...
    d = DiscoverPing()
    d.run(hostnames=domains, repeat=1, num_threads=5)
    d.join()
    while not d.done():
        d.process()
    d.draw()
